...
router.get("/all", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const allEnrollments = await allEnrollmentsQuery;
...
router.patch("/:enrollmentId/approve", authenticate, async (req: AuthRequest, res) => {
  try {
    if (req.userRole !== "admin") {
      return res.status(403).json({ error: "Forbidden: Admin access required" });
    }
...
router.patch("/:enrollmentId/reject", authenticate, async (req: AuthRequest, res) => {
  try {
    if (req.userRole !== "admin") {
      return res.status(403).json({ error: "Forbidden: Admin access required" });
    }
...
router.get("/pending-count", authenticate, async (req: AuthRequest, res) => {
  try {
    if (req.userRole !== "admin") {
